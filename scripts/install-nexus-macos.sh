...
# ================================================================
#  Nexus Memory Trust - macOS Installer
# ================================================================
...
print_header() {
    echo ""
    echo -e "${CYAN}================================================================${NC}"
    echo -e "${CYAN}           Nexus Memory Trust - macOS Installer                ${NC}"
    echo -e "${CYAN}================================================================${NC}"
    echo ""
}
...
download_binary() {
    ...
    case $ARCH in
        arm64)
            ASSET_NAME="nexus-macos-arm64"
            ;;
        x86_64)
            ASSET_NAME="nexus-macos-x64"
            ;;
        *)
    ...
    if curl -L -o "$INSTALL_DIR/nexus-icon.png" "$ICON_URL" 2>/dev/null; then
...
<dict>
    ...
    <string>com.nexus.memorytrust</string>
    ...
    <string>nexus-icon</string>
</dict>
</plist>
EOF
    
    # Copy binary
    cp "$INSTALL_DIR/$BINARY_NAME" "$MACOS_DIR/$BINARY_NAME"
    
    # Copy icon if available
    if [ -f "$INSTALL_DIR/nexus-icon.png" ]; then
        cp "$INSTALL_DIR/nexus-icon.png" "$RESOURCES_DIR/nexus-icon.png"
        print_success "Application bundle created with icon: $APP_BUNDLE"
    else
        print_success "Application bundle created (default icon): $APP_BUNDLE"
    fi
}
...
rm -rf "/Applications/Nexus Memory Trust.app"
...
if [ -f "$HOME/.bash_profile" ]; then
    sed -i '' '/export PATH.*\.local\/share\/nexus/d' "$HOME/.bash_profile"
fi
...
echo "   • Open Applications folder and click 'Nexus Memory Trust'"
echo "   • Or run: nexus"
echo ""
