...
pub fn interpret_image(path: &Path, content: &[u8]) -> ImageInterpretation {
    ...
    entity
        .metadata
        .insert("kind".into(), serde_json::json!("image"));
    entity
        .metadata
        .insert("format".into(), serde_json::json!(format));
    if let Some(w) = width {
        entity.metadata.insert("width".into(), serde_json::json!(w));
    }
...
fn detect_dimensions(content: &[u8], ext: &str) -> (Option<u32>, Option<u32>) {
    ...
    match ext {
        "png"
            ...
            if content.len() >= 24 => {
                let width =
                    u32::from_be_bytes([content[16], content[17], content[18], content[19]]);
                let height =
                ...
                if content[i] == 0xFF {
                    ...
                    match marker {
                        ...
                        _ => {
                            ...
                            let seg_len =
                                u16::from_be_bytes([content[i + 2], content[i + 3]]) as usize;
                            if seg_len < 2 {
                                break;
                            }
                            i += 2 + seg_len;
        ...
        "bmp"
            ...
            if content.len() >= 26 => {
                let width =
                    i32::from_le_bytes([content[18], content[19], content[20], content[21]]) as u32;
                let height =
                    i32::from_le_bytes([content[22], content[23], content[24], content[25]])
                        .unsigned_abs();
                return (Some(width), Some(height));
            }
        "webp"
            ...
            if let Some(viewbox) = text.find("viewBox") {
                ...
                if let Some(start) = rest.find('"').or_else(|| rest.find('\'')) {
                    ...
                    if let Some(end) = after_quote.find('"').or_else(|| after_quote.find('\'')) {
                        ...
                        if coords.len() == 4
                            && let (Ok(_), Ok(h)) =
                                (coords[2].parse::<f64>(), coords[3].parse::<f64>())
                            {
