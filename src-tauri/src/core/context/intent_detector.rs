...
impl IntentDetector {
    ...
    ///
    /// Delegates to the shared `core::text::normalize_search_words` so intent
    /// detection and graph/memory search normalize queries identically.
    pub fn extract_keywords(&self, query: &str) -> Vec<String> {
        crate::core::text::normalize_search_words(query)
    }
