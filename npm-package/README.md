...
- 143 инструмента для работы с графом и файлами
...
- 13 версионированных миграций, вкомпилированных в бинарник
...
│  13 миграций, вкомпилированных в бинарник                 │
└──────────────────────────────────────────────────────────┘
```
...
### v1.2.0 — флагманский релиз: экраны всех систем и бенчмарки

| # | Область | Что сделано |
|:---:|:---|:---|
| 1 | Повторение (System 3) | Экран `RehearsalView`: план повторения, цикл, канонические воспоминания, консолидация |
| 2 | Файрвол (System 4) | Экран `FirewallView`: пробник контента, правила, карантин, политики агентов |
| 3 | Контекстная цепочка (System 5) | Секция Why-Chains во `FlightView`: «почему ИИ так сказал» |
| 4 | Контекст-Лаб (System 6) | Экран `ContextLabView`: A/B/C-стратегии с метриками и победителем |
| 5 | Паспорт агента (System 7) | Экран `PassportView`: паспорта, активация, права |
| 6 | Скиллы (System 8) | Экран `SkillsView` + Skill Genesis (сканирование, approve/reject) |
| 7 | Предиктор (System 9) | Экран `PredictiveView`: следующий запрос, прогрев сущностей |
| 8 | Карта знаний (System 10) | Экран `KnowledgeMapView`: карта вокруг сущности четырьмя кольцами |
| 9 | Диагностика | Экран `DiagnosticsView`: «Run checks» реально перезапускает проверки (remount через `runId`), спиннер busy-состояния |
| 10 | MCP-API | 100 → 143 инструмента |
| 11 | Бенчмарки | `BENCHMARKS.md`: реальные измерения на 1320 файлах (R@5 0.95, экономия токенов 94.2%, security 55/55) |
| 12 | Тесты | Полный прогон: `cargo test` 978 lib + 56 интеграционных, vitest 41, Playwright 10 — **1085 зелёных** |

### v1.1.0 — когнитивные слои памяти

| # | Область | Что сделано |
|:---:|:---|:---|
| 1 | Таксономия | 6 когнитивных слоёв (Working → Strategic), миграция V18, бэкфилл |
| 2 | Классификатор | Детерминированный сигнатурный скоринг (RU+EN), провенанс слоя, ручной выбор закрепляет |
| 3 | MCP-API | 74 → 100 инструментов (`nexus_layers_list/set/reclassify/history/stats`) |
| 4 | UI | Слои и провенанс в `MemoryCard`/`MemoryDetail`, лестница слоёв, Reclassify |
| 5 | Тесты | 9 unit + 9 фронтенд + 3 интеграционных; `reclassify` не перезаписывает закреплённый слой |
